...
export const ProductsInCart = () => {
  ...
  const { cart, updateProductQuantity, removeProduct, getCartTotal } = useCartStore(state => state)
          ...
          <div key={`${product.id}-${product.selectedAttributes ? JSON.stringify(product.selectedAttributes) : ''}`} className="flex mb-5 pb-5 border-b border-gray-200 last:border-0">
            ...
            <div className="flex-1">
              ...
              <div className="mb-2">
                ...
                <p className={`font-bold ${hasDiscount && totalOriginalPrice > totalPrice ? 'text-green-600' : ''}`}>
              ...
              />
              <button onClick={() => removeProduct(product)} className="underline mt-3 text-red-600 hover:text-red-800">Remover</button>
            </div>
          </div>
        );
      })}
