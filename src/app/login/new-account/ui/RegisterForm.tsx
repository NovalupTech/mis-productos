...
export const RegisterForm = () => {
    ...
    const { register, handleSubmit, formState: { errors }, formState } = useForm<InputsValues>();
    ...
    const onSubmit: SubmitHandler<InputsValues> = async (data) => {
        ...
        if(user.ok){
            ...
            const state = await authenticateUser(undefined, formData);
            if(state === 'success')
		...
		<form onSubmit={ handleSubmit(onSubmit)} className="flex flex-col space-y-4">
            ...
            <div className="flex flex-col">
				...
				<input
					className={clsx(
						"px-4 py-3 border border-gray-300 bg-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent",
						{
							"border-red-500 focus:ring-red-500": errors.phoneNumber && errors.phoneNumber.message
						}
					)}
					type="number"
					id="phoneNumber"
					{...register("phoneNumber", { required: true, pattern: /^\d{10}$/i })}
				/>
			</div>
			...
			<button 
				...
				className={clsx(
					...
					{
						'bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2': !formState.isSubmitting,
						'bg-gray-400 text-gray-200 cursor-not-allowed': formState.isSubmitting,
					}
				)}
			...
			<Link 
				href="/" 
				className="text-center py-3 px-4 border border-gray-300 rounded-lg hover:bg-blue-600 transition-colors font-medium bg-blue-500 text-white flex items-center justify-center gap-2"
			>
